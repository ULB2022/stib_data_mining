...
app.layout = html.Div(
    [
        ...
        html.Div(
            children=[
                html.Button("Add Chart", id="add-graph", n_clicks=0,className="btn btn-primary"),
                html.Button("Add Table", id="add-table", n_clicks=0,className="btn btn-light"),
                html.Button("Add Map", id="add-map", n_clicks=0,className="btn btn-success"),
            ]
...
@app.long_callback(output=[Output('l-dropdown', 'options'),
                Output('s-dropdown', 'options'),
                Output('date-dropdown', 'options'),
                Output('s-dropdown', 'value'),
                Output('date-dropdown', 'value'),
                ],
              inputs=[dash.dependencies.Input('s-dropdown', 'value'),
            ...
            #   dash.dependencies.Input('date-dropdown', 'value'),
              ...
              state=[State('date-dropdown', 'value'),
                    ...
                    manager=long_callback_manager,)
def change_my_dropdown_options(initial_stop,initial_line,initial_direction,initial_transport,date_value,stop_value,transport_value):
    # print(s,d,l,t)
    ...
    if "t-dropdown" == ctx.triggered_id:
        ...
        stops = line_stop_mapping["all_lines"][str(initial_line)][initial_direction]
        dates = timejoin.get_dates(route_line_mapping[str(initial_line)],initial_stop.split('-',1)[0],int(initial_direction))
    ...
    # l_option = mode_line_mapping[t]
    # options = []
    # if l in l_option:
    #     options = line_stop_mapping["all_lines"][str(l)][d]
    # else:
    #     options = line_stop_mapping["all_lines"][str(l_option[0])][d]
    # route_id = route_line_mapping[str(l)]

    # date_options = timejoin.get_dates(route_id,"MERODE",1)
    # options = line_stop_mapping["all_lines"]["1"][str(d)]
    # options = line_stop_mapping["all_lines"]["1"]["1"]
    # print("print",lines,stops,dates,stop_value,date_value)
...
# )
# def display_table(n_clicks, div_children):
#     new_child = html.Div(
#         style={
#             "width": "49%",
#             "padding": 10,
...
#         draggable='true',
#         children=[
#             dash_table.DataTable(data =[], id={"type": "dynamic-table", "index": n_clicks}),
#         ],
#     )
...
@app.callback(
    Output("container", "children"),
    [Input("add-graph", "n_clicks"),Input("add-table", "n_clicks")],
    [State("container", "children"),State('date-dropdown', 'value'),State('s-dropdown', 'value'),
                    State('d-dropdown', 'value'),
                    ],
                    manager=long_callback_manager,
)
def display_graphs(n_clicks,n_clicks_table, div_children,initial_date, initial_stop,initial_direction):
    # print(n_clicks_table,n_clicks)
    new_child = None
    ...
    if "add-graph" == ctx.triggered_id:
        fig = timejoin.map_gtfs(stop_id=stop_id,stop_name=stop_name, direction=direction,cd_date= date)
        new_child = html.Div(
            style={
                "width": "100%",
        ...
        df =  timejoin.map_gtfs(stop_id=stop_id,stop_name=stop_name, direction=direction,cd_date=date, send_dataframe=True)
        # print(df.dtypes)
        print("in table",df.head())
        df = df[["route_id","trip_headsign","stop_sequence", "processed_arrival_datetime","mapped_time","delay_in_secs"]]
        new_child = html.Div(
        style={
            # "width": "47%",
            # "display": "inline-block",
